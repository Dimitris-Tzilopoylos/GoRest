...
const CREATE_UNIQUE_INDEX = "ALTER TABLE %s.%s ADD CONSTRAINT %s  %s (%s)"
const CREATE_FOREIGN_INDEX = "ALTER TABLE %s.%s ADD CONSTRAINT %s FOREIGN KEY (%s) REFERENCES %s.%s(%s) %s %s"
const DROP_INDEX = `ALTER TABLE %s.%s DROP CONSTRAINT %s CASCADE;`
const DROP_UNIQUE_INDEX = `DROP INDEX IF EXISTS %s CASCADE;`
